...
これならば、2カラムレイアウトに必要なCSSは全て`Wrapper`に属しており、`LeftColumn`や`RightColumn`はカラムの中身に集中しています。責務の観点からもこのようにCSSを分割する方が適切です。

ところで、元々の例に比べるとこちらは`div`の層が一つ増えていますね。次のようにするのはだめなのかと思われるかもしれません。
...
最初の例のように`Wrapper`の中で`div`の層を作ることで、スタイリングが自身の中で完結したコンポーネントとすることができます。

## まとめ

ReactでCSSを書くときはコンポーネント間の依存を発生させないように書きましょう。

余談ですが、筆者が最近作った[react-wc](https://blog.uhy.ooo/entry/2020-10-03/react-wc/)はこれを促進する設計になっています。ぜひ使ってみてください（宣伝）。